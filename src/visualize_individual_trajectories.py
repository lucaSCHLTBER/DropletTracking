...
class Visualizer:
    ...
    def update_display(self):
        ...
        if self.all_channels:
            ...
            self.axarr[0, 0].plot([2, 2 + 20], [45, 45], color='red', lw=2, zorder=2)
            self.axarr[0, 0].text(2 + 20 / 2, 46, '20 pixels', color='red', ha='center', va='top', zorder=3, fontsize=7)

        plt.draw()
        # Update the label text
        self.row_info_text.set_text(f"Row: {self.current_idx + 1} / {self.number_of_rows}     |"
                                    f"     20 pixels = 39,57 qm (the shown patches are {2*self.cfg.radius}x{2*self.cfg.radius} pixels)")
